...
def create_dull_pd_matrix(dullvalue=0.0, dullname="A", startdate=pd.datetime(1970, 1, 1).date(),
...
def basic_opt(std, corr, mus):
    ...
    return minimize(neg_SR_riskfree, start_weights, (sigma, mus), method='SLSQP', bounds=bounds, constraints=cdict,
                    tol=0.00001)
...
def handcrafted(returns, equalisevols=True, default_vol=0.2):
    ...
    wts_to_use = HANDCRAFTED_WTS[
        (HANDCRAFTED_WTS.c1 == c1) & (HANDCRAFTED_WTS.c2 == c2) & (HANDCRAFTED_WTS.c3 == c3)].irow(0)
...
def bootstrap_portfolio(returns_to_bs, monte_carlo=200, monte_length=250, equalisemeans=False, equalisevols=True,
    ...
    for unused_index in range(monte_carlo):
        ...
        weight = markosolver(returns, equalisemeans=equalisemeans, equalisevols=equalisevols, default_vol=default_vol,
...
def optimise_over_periods(data, date_method, fit_method, rollyears=20, equalisemeans=False, equalisevols=True,
    ...
    for fit_tuple in fit_periods:
        ...
        print("Fitting data for {start} to {end}".format(start=str(fit_tuple[2]), end=str(fit_tuple[3])))
